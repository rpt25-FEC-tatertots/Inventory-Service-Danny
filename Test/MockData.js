module.exports = [{
  productID: 1,
  price: 139,
  colors: [
    {
      colorName: 'El Cap Khaki',
      colorAbbreviation: 'ELKH',
      thumbnail: 'https://www.patagonia.com/dw/image/v2/BDJB_PRD/on/demandware.static/-/Sites-patagonia-master/default/dwbe5ef79d/images/hi-res/22801_ELKH.jpg?sw=800&sh=800&sfrm=png&q=95&bgcolor=f6f6f6',
      discount: null,
      hexCode: '#a5e766',
      inventory: [
        {
          size: 'XXS',
          quantity: 8,
        },
        {
          size: 'XS',
          quantity: 0,
        },
        {
          size: 'S',
          quantity: 3,
        },
        {
          size: 'M',
          quantity: 6,
        },
        {
          size: 'L',
          quantity: 8,
        },
        {
          size: 'XL',
          quantity: 3,
        },
        {
          size: 'XXL',
          quantity: 3,
        },
      ],
    },
    {
      colorName: 'Black',
      colorAbbreviation: 'BLK',
      thumbnail: 'https://www.patagonia.com/dw/image/v2/BDJB_PRD/on/demandware.static/-/Sites-patagonia-master/default/dw9b634d4e/images/hi-res/22801_BLK.jpg?sw=800&sh=800&sfrm=png&q=95&bgcolor=f6f6f6',
      discount: null,
      hexCode: '#49d27b',
      inventory: [
        {
          size: 'XXS',
          quantity: 4,
        },
        {
          size: 'XS',
          quantity: 8,
        },
        {
          size: 'S',
          quantity: 8,
        },
        {
          size: 'M',
          quantity: 3,
        },
        {
          size: 'L',
          quantity: 6,
        },
        {
          size: 'XL',
          quantity: 1,
        },
        {
          size: 'XXL',
          quantity: 1,
        },
      ],
    },
    {
      colorName: 'New Navy',
      colorAbbreviation: 'NENA',
      thumbnail: 'https://www.patagonia.com/dw/image/v2/BDJB_PRD/on/demandware.static/-/Sites-patagonia-master/default/dw6b7fd00a/images/hi-res/22801_NENA.jpg?sw=800&sh=800&sfrm=png&q=95&bgcolor=f6f6f6',
      discount: null,
      hexCode: '#5bd758',
      inventory: [
        {
          size: 'XXS',
          quantity: 6,
        },
        {
          size: 'XS',
          quantity: 9,
        },
        {
          size: 'S',
          quantity: 3,
        },
        {
          size: 'M',
          quantity: 4,
        },
        {
          size: 'L',
          quantity: 8,
        },
        {
          size: 'XL',
          quantity: 9,
        },
        {
          size: 'XXL',
          quantity: 0,
        },
      ],
    },
    {
      colorName: 'Feather Grey',
      colorAbbreviation: 'FEA',
      thumbnail: 'https://www.patagonia.com/dw/image/v2/BDJB_PRD/on/demandware.static/-/Sites-patagonia-master/default/dwbc5f5f68/images/hi-res/22801_FEA.jpg?sw=800&sh=800&sfrm=png&q=95&bgcolor=f6f6f6',
      discount: null,
      hexCode: '#38c03f',
      inventory: [
        {
          size: 'XXS',
          quantity: 0,
        },
        {
          size: 'XS',
          quantity: 7,
        },
        {
          size: 'S',
          quantity: 9,
        },
        {
          size: 'M',
          quantity: 3,
        },
        {
          size: 'L',
          quantity: 8,
        },
        {
          size: 'XL',
          quantity: 9,
        },
        {
          size: 'XXL',
          quantity: 6,
        },
      ],
    },
    {
      colorName: 'Oak Grove Green',
      colorAbbreviation: 'OAGR',
      thumbnail: 'https://www.patagonia.com/dw/image/v2/BDJB_PRD/on/demandware.static/-/Sites-patagonia-master/default/dw55358486/images/hi-res/22801_OAGR.jpg?sw=800&sh=800&sfrm=png&q=95&bgcolor=f6f6f6'
      discount: null,
      hexCode: '#bc594f',
      inventory: [
        {
          size: 'XXS',
          quantity: 0,
        },
        {
          size: 'XS',
          quantity: 8,
        },
        {
          size: 'S',
          quantity: 6,
        },
        {
          size: 'M',
          quantity: 1,
        },
        {
          size: 'L',
          quantity: 3,
        },
        {
          size: 'XL',
          quantity: 7,
        },
        {
          size: 'XXL',
          quantity: 4,
        },
      ],
    }, {
      colorName: 'Piton Purple',
      colorAbbreviation: 'PTPL',
      thumbnail:'https://www.patagonia.com/dw/image/v2/BDJB_PRD/on/demandware.static/-/Sites-patagonia-master/default/dw631e3a2f/images/hi-res/22801_PTPL.jpg?sw=800&sh=800&sfrm=png&q=95&bgcolor=f6f6f6',
      discount: null,
      hexCode: '#800080',
      inventory: [
        {
          size: 'XXS',
          quantity: 0,
        },
        {
          size: 'XS',
          quantity: 4,
        },
        {
          size: 'S',
          quantity: 7,
        },
        {
          size: 'M',
          quantity: 4,
        },
        {
          size: 'L',
          quantity: 1,
        },
        {
          size: 'XL',
          quantity: 2,
        },
        {
          size: 'XXL',
          quantity: 8,
        },
      ],
    },
  ],
}, {
  productID: 2,
  price: 79,
  colors: [
    {
      colorName: 'Noble Grey',
      colorAbbreviation: 'NGRY',
      discount: null,
      hexCode: '#70c102',
      inventory: [
        {
          size: 'XS',
          quantity: 5,
        },
        {
          size: 'S',
          quantity: 1,
        },
        {
          size: 'M',
          quantity: 8,
        },
        {
          size: 'L',
          quantity: 6,
        },
        {
          size: 'XL',
          quantity: 6,
        },
        {
          size: 'XXL',
          quantity: 9,
        },
      ],
    },
    {
      colorName: 'Hex Grey',
      colorAbbreviation: 'HEXG',
      discount: null,
      hexCode: '#ce4897',
      inventory: [
        {
          size: 'XS',
          quantity: 3,
        },
        {
          size: 'S',
          quantity: 2,
        },
        {
          size: 'M',
          quantity: 3,
        },
        {
          size: 'L',
          quantity: 5,
        },
        {
          size: 'XL',
          quantity: 4,
        },
        {
          size: 'XXL',
          quantity: 4,
        },
      ],
    },
    {
      colorName: 'Ink Black',
      colorAbbreviation: 'INKB',
      discount: 0.53,
      hexCode: '#99af39',
      inventory: [
        {
          size: 'XS',
          quantity: 2,
        },
        {
          size: 'S',
          quantity: 2,
        },
        {
          size: 'M',
          quantity: 8,
        },
        {
          size: 'L',
          quantity: 5,
        },
        {
          size: 'XL',
          quantity: 8,
        },
        {
          size: 'XXL',
          quantity: 8,
        },
      ],
    },
    {
      colorName: 'Kale Green',
      colorAbbreviation: 'KAGR',
      discount: null,
      hexCode: '#15f605',
      inventory: [
        {
          size: 'XS',
          quantity: 5,
        },
        {
          size: 'S',
          quantity: 7,
        },
        {
          size: 'M',
          quantity: 4,
        },
        {
          size: 'L',
          quantity: 6,
        },
        {
          size: 'XL',
          quantity: 2,
        },
        {
          size: 'XXL',
          quantity: 5,
        },
      ],
    },
    {
      colorName: 'Smolder Blue',
      colorAbbreviation: 'SMDB',
      discount: null,
      hexCode: '#59ee36',
      inventory: [
        {
          size: 'XS',
          quantity: 0,
        },
        {
          size: 'S',
          quantity: 5,
        },
        {
          size: 'M',
          quantity: 8,
        },
        {
          size: 'L',
          quantity: 9,
        },
        {
          size: 'XL',
          quantity: 1,
        },
        {
          size: 'XXL',
          quantity: 1,
        },
      ],
    },
  ],
}, {
  productID: 3,
  price: 79,
  colors: [
    {
      colorName: 'Santa Paula: Mountain Yellow',
      colorAbbreviation: 'SPMY',
      discount: null,
      hexCode: '#a16823',
      inventory: [
        {
          size: 'XXS',
          quantity: 8,
        },
        {
          size: 'XS',
          quantity: 8,
        },
        {
          size: 'S',
          quantity: 9,
        },
        {
          size: 'M',
          quantity: 4,
        },
        {
          size: 'L',
          quantity: 4,
        },
        {
          size: 'XL',
          quantity: 6,
        },
        {
          size: 'XXL',
          quantity: 2,
        },
      ],
    },
    {
      colorName: 'Grange: Superior Blue',
      colorAbbreviation: 'GRSB',
      discount: null,
      hexCode: '#285a9f',
      inventory: [
        {
          size: 'XXS',
          quantity: 0,
        },
        {
          size: 'XS',
          quantity: 9,
        },
        {
          size: 'S',
          quantity: 6,
        },
        {
          size: 'M',
          quantity: 2,
        },
        {
          size: 'L',
          quantity: 7,
        },
        {
          size: 'XL',
          quantity: 8,
        },
        {
          size: 'XXL',
          quantity: 9,
        },
      ],
    },
    {
      colorName: 'Rotation: Big Sky Blue',
      colorAbbreviation: 'ROBK',
      discount: null,
      hexCode: '#7e27a8',
      inventory: [
        {
          size: 'XXS',
          quantity: 8,
        },
        {
          size: 'XS',
          quantity: 1,
        },
        {
          size: 'S',
          quantity: 3,
        },
        {
          size: 'M',
          quantity: 4,
        },
        {
          size: 'L',
          quantity: 6,
        },
        {
          size: 'XL',
          quantity: 8,
        },
        {
          size: 'XXL',
          quantity: 4,
        },
      ],
    },
    {
      colorName: 'Collective: Big Sky Blue',
      colorAbbreviation: 'CLBU',
      discount: null,
      hexCode: '#8aeb03',
      inventory: [
        {
          size: 'XXS',
          quantity: 9,
        },
        {
          size: 'XS',
          quantity: 0,
        },
        {
          size: 'S',
          quantity: 7,
        },
        {
          size: 'M',
          quantity: 8,
        },
        {
          size: 'L',
          quantity: 4,
        },
        {
          size: 'XL',
          quantity: 6,
        },
        {
          size: 'XXL',
          quantity: 1,
        },
      ],
    },
    {
      colorName: 'Ikat Rows: Stone Blue',
      colorAbbreviation: 'IRSB',
      discount: 0.77,
      hexCode: '#cbe686',
      inventory: [
        {
          size: 'XXS',
          quantity: 4,
        },
        {
          size: 'XS',
          quantity: 4,
        },
        {
          size: 'S',
          quantity: 2,
        },
        {
          size: 'M',
          quantity: 6,
        },
        {
          size: 'L',
          quantity: 5,
        },
        {
          size: 'XL',
          quantity: 6,
        },
        {
          size: 'XXL',
          quantity: 2,
        },
      ],
    },
  ],
}];
